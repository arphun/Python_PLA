...
def PLA(x, y, w_vector, order, data_number, dimension, uptime):
    index = 0 # the index of x we are checking
    last_corrected = data_number - 1 # the index of the last corrected data 
    while(True):
...
y = raw_data[:,4:]
update_list = []
for trial in range(0,1126):
#initialize w
    w_vector = np.zeros((1,dimension))
#set order
    order = np.random.choice(data_number, data_number, replace=False)
#do PLA
    uptime = 0
    uptime = PLA(x, y, w_vector, order, data_number, dimension, uptime)
    update_list.append(uptime)
    print('trial:', trial, w_vector, 'updated_time is', uptime)
